...
ReFxRCON holds RCON passwords for game servers, so treat a vulnerability report as urgent.
...
Good practice when hosting: keep the RCON listener on `127.0.0.1` next to ReFxRCON, set `ORIGIN` and
